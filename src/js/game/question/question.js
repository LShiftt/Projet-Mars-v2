import questionsRaw from "@/question.json";
...
questionsData = questionsRaw.questions;
loadNewQuestion();
  ...
  currentQuestion.choices.forEach((choice) => {
    ...
    btn.addEventListener("click", () => {
      ...
      document.querySelectorAll(".btn_check").forEach((button) => {
        ...
        button.style.backgroundColor = verifyAnswer(
          button.innerHTML,
      ...
      if (
        progress.value < 10 &&
        document.querySelectorAll(".life").length > 0
      ) {
...
function augmenterProgression() {
  let valeur = localStorage.getItem("good_answer")
    ? parseInt(localStorage.getItem("good_answer"))
    : 0;
  valeur += 1;
...
function WinGame(message) {
  let valeur = localStorage.getItem("nbr_win")
    ? parseInt(localStorage.getItem("nbr_win"))
    : 0;
  valeur += 1;
